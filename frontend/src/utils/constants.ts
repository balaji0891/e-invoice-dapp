...
export const INVOICE_MANAGER_ABI = [
  ...
  "function createInvoice(address _recipient, string memory _description, bytes memory _encryptedAmount, bytes calldata _inputProof, uint256 _dueDate) external returns (uint256)",
  "function payInvoice(uint256 _invoiceId) external",
  "function cancelInvoice(uint256 _invoiceId) external",
  "function getEncryptedAmount(uint256 _invoiceId) external view returns (bytes memory)",
  "function getInvoiceDetails(uint256 _invoiceId) external view returns (uint256 id, address sender, address recipient, string memory description, uint256 dueDate, uint8 status, uint256 createdAt, uint256 paidAt)",
